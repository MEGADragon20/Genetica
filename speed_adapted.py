...
Food count is the number of blobs
"""
...
class Screen(arcade.Window):
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        if self.playing == False:
            ...
            current_min = 10
            for i in self.blobs:
                if i.speed > current_max:
                    current_max = i.speed
                if i.speed < current_min:
                    current_min = i.speed
            print("speeds", current_max, current_min)
            self.playing = True
            self.setup_foods()
...
class Blob(arcade.Sprite):
    ...
    def duplicate(self):
            return Blob(make_position(), self.foods, plusminus(self.speed))
