...
def check_if_eddible(predator_blob, prey_blob):
    delta_size = predator_blob.size - prey_blob.size
    if delta_size >= 0.5:
        return True
    return False

def calculate_energy(speed, size):
    return speed * size
...
class Screen(arcade.Window):
    ...
    def setup(self):
        ...
        for _ in range(100):  
            ...
            blob = Blob(position, self.foods, self.blobs)
    ...
    def on_update(self, delta_time):
        ...
        if self.playing == False:
            ...
            py = {round(x, 1): 0 for x in [i / 10 for i in range(1, 50)]}
            sy = {round(x, 1): 0 for x in [i / 10 for i in range(1, 50)]}
...
class Blob(arcade.Sprite):
    def __init__(self, position, foods, blobs, speed = 1, precision = 1, size = 1):
        super().__init__("data/blob.png", scale=math.sqrt(size))
    ...
    def duplicate(self):
        ...
        new_size = plusminus(self.size)
        new_blob = Blob(make_position(), self.foods, self.blobs, new_speed, new_precision, new_size)
        return new_blob
    ...
    def on_update(self, delta_time=1 / 60):
        if self.energy >= 0:
            self.move_around()
            self.energy -= calculate_energy(self.speed, self.size)
            for i in self.foods:
                if arcade.check_for_collision(self, i):
                    self.foods.remove(i)
                    self.food += 1
            for i in self.blobs:
                if i != self and arcade.check_for_collision(self, i) and check_if_eddible(self, i):
                    print("mhmmmm yummy canibalism")
                    i.kill()
                    self.food += 1
...
def main():
    ...
    for i in range(5):
        for i in range(len(td)):
            ...
            ax1.plot(td[i][0], td[i][1], color = "red")
            ax2.plot(sd[i][0], sd[i][1], color = "blue")
            ax3.plot(pd[i][0], pd[i][1], color = "yellow")
            ax1.set(label = "b")
            plt.pause(1)
