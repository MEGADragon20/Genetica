...
class Screen(arcade.Window):
    ...
    def on_update(self, delta_time):
        ...
        if self.playing == False:
            ...
            sy = {round(x, 1): 0 for x in [i / 10 for i in range(1, 30)]}
...
class Blob(arcade.Sprite):
    def __init__(self, position, foods, speed = 1, precision = 1, size = 1):
    ...
    def duplicate(self):
        ...
        new_blob = Blob(make_position(), self.foods, new_speed, new_precision, new_size)
        return new_blob
...
def main():
    ...
    for i in range(5):
        for i in range(len(td)):
            ...
            ax3.plot(sd[i][0], sd[i][1], color = "blue")
            plt.pause(1)
